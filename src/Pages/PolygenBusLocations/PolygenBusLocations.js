...
import Map from "../../Components/PolygenBusLocationsMap/Map";
import "./PolygenBusLocations.scss";
...
const AllBusLocations = () => {
  ...
  useEffect(() => {
    const wsClient = new WebSocket(
      `${appConfig.socketBaseAddress}websocket/getRegionTracking`
    );
    wsClient.onopen = () => {
  ...
  const getPolygenBusData = (polygen) => {
    setIsLoading(true);
    const polygenArr = polygen.map((item) => [item.lat, item.lng]);
    ws.send(
      JSON.stringify({
        messageType: "getRegionTracking",
        payloadType: "getRegionTracking",
        payload: polygenArr,
      })
    );
  };
  ...
  return (
    <section
      className={`all-bus-polygen-locations-container ${appConfig.language.direction}`}
    >
      <div className="map-contianer">
        {isLoading ? (
          ...
          <Map
            ...
            center={mapCenter}
            zoom={mapZoom}
            onPolygenDraw={(polygen) => getPolygenBusData(polygen)}
          />
        )}
      </div>
