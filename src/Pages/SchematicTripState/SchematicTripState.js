...
const SchematicTripState = () => {
  ...
  useEffect(() => {
    ws.onmessage = evt => {
      ...
      console.log(message)
      const busStopsTemp = message.payload.inboundPoints.filter(item => item.stopName !== null);
      ...
      const busesTemp = message.payload.inboundBusList.map((bus, indexBus) => {
        ...
        busStopsTempFinal.map((busStop, indexBusStop) => {
          if ((busStop.order<= bus.pointOrder) && (busStopsTempFinal[(busStopsTempFinal.length === indexBusStop +1) ? indexBusStop : indexBusStop+1].order >= bus.pointOrder)) {
            marginLeft = (parseInt(
              forwardLineRef.current.offsetWidth -
              20 +
              parseInt(forwardLineRef.current.offsetWidth - 20) /
            ...
            busStopsTempFinal.length) * indexBusStop;
          }
        })
        return {
          ...bus,
      ...
      //outBound
      const outboundBusStopsTemp = message.payload.outboundPoints.filter(item => item.stopName !== null);
      const outboundBusStopsTempFinal = outboundBusStopsTemp.map((busStop, index) => {
        return {
          ...busStop,
          marginRight:
          parseInt(
            ...
            40 -
            parseInt(backwardLineRef.current.offsetWidth - 20) /
            busStopsTemp.length
          ) /
              busStopsTemp.length *
            index,
      ...
      const outbusesTemp = message.payload.outboundBusList.map((bus, indexBus) => {
        var marginRight
        outboundBusStopsTempFinal.map((busStop, indexBusStop) => {
          if ((busStop.order<= bus.pointOrder) && (outboundBusStopsTempFinal[(outboundBusStopsTempFinal.length === indexBusStop +1) ? indexBusStop : indexBusStop+1].order >= bus.pointOrder)) {
    ...
    <section className="SchematicTripState">
      ...
      <div className="card">
        ...
        <div className="line backwards" ref={backwardLineRef}>
          {outboundBuses.reverse().map((bus, index) => {
            return (
              <div key={index} className="bus" style={{ right: bus.marginRight}}>
                <div className="pos pin">
                  <div
                    className="pin-marker"
                  // style={{ background: getRandomColor() }}
          ...
          {outboundBusStops.map((busStop, index) => {
            return (
              <div
                key={index}
                className="bus-stop"
                style={{ right: busStop.marginRight }}
              >
                <div className="bus-stop-counter">{busStop.stopCode}</div>
                <div className="bus-stop-name">{busStop.stopName}</div>
              </div>
